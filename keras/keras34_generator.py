...
# 컨볼루션 신경망의 설정
model = Sequential()
model.add(Conv2D(100, kernel_size = (3, 3), input_shape = (28, 28, 1), activation='relu'))
...
data_generator = ImageDataGenerator(
    rotation_range=20, # 회전값
    width_shift_range=0.02, # 넓이 설정
    height_shift_range=0.02, # 높이 설정
    horizontal_flip=True # 수평값 
)
model.fit_generator(data_generator.flow(X_train, Y_train, batch_size=600),
                    steps_per_epoch = len(X_train) // 3,   # steps_per_epoch * batch_size  => 몇 배로 증폭 시킬 것인가?? <(len(X_train) // 32) * 32>
                    epochs=20,
