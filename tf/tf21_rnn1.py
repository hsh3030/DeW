...
enc = OneHotEncoder()
enc.fit(_data)
_data = enc.transform(_data).toarray().astype('float32') # OneHot을 하면 순서대로 정렬이 된다. float64 -> float32 변환
...
# 데이터 구성
# x : (batch_size, sequence_length, input_dim) 1, 6, 6
...
cell = tf.nn.rnn_cell.BasicLSTMCell(hidden_size) # 출력 사이즈 hidden_size = output 입력
...
prediction = tf.argmax(outputs, axis=2) # OneHot data를 원래 data로 변환 = argmax
